...
! function() {
    function t(i) {
        ...
        this._canvas = i = "string" == typeof i ? document.getElementById(i) : i, this._ctx = i.getContext("2d"), this._width = i.width, this._height = i.height, this._max = 1, this.clear()
    }
    t.prototype = {
        defaultRadius: 25,
        defaultGradient: {
        ...
        draw: function(t) {
            ...
            for (var a, s = 0, e = this._data.length; s < e; s++) a = this._data[s], i.globalAlpha = Math.max(a[2] / this._max, t || .05), i.drawImage(this._circle, a[0] - this._r, a[1] - this._r);
        ...
        _colorize: function(t, i) {
            for (var a, s = 3, e = t.length; s < e; s += 4)(a = 4 * t[s]) && (t[s - 3] = i[a], t[s - 2] = i[a + 1], t[s - 1] = i[a + 2])
        }
    }, window.simpleheat = t
        ...
        if (this._map) {
            ...
            for (t = 0, i = this._latlngs.length; t < i; t++)
                if (a = this._map.latLngToContainerPoint(this._latlngs[t]), m.contains(a)) {
                    e = Math.floor((a.x - w) / p) + 2, n = Math.floor((a.y - y) / p) + 2;
                    var x = void 0 !== this._latlngs[t].alt ? this._latlngs[t].alt : void 0 !== this._latlngs[t][2] ? +this._latlngs[t][2] : 1;
                    r = x * f, g[n] = g[n] || [], s = g[n][e], s ? (s[0] = (s[0] * s[2] + a.x * r) / (s[2] + r), s[1] = (s[1] * s[2] + a.y * r) / (s[2] + r), s[2] += r) : g[n][e] = [a.x, a.y, r]
                } for (t = 0, i = g.length; t < i; t++)
                if (g[t])
                    for (h = 0, o = g[t].length; h < o; h++)(s = g[t][h]) && _.push([Math.round(s[0]), Math.round(s[1]), Math.min(s[2], c)]);
